...
export const chatSessions = createTable(
  ...
  (t) => [
    ...
    check(
      ...
      sql`${t.locationCount} between 0 and 3`,
    ),
  ],
);
...
export const chatRecommendations = createTable(
  ...
  (t) => [
    ...
    index("chat_recommendations_review_backed_idx").on(
      t.reviewBacked,
      t.createdAt,
    ),
    check(
...
export const sourceRuns = createTable(
  ...
  (t) => [
    ...
    check(
      ...
      sql`${t.status} in ('ok','error','partial')`,
    ),
  ],
);
...
export const chatMemoryEvents = createTable(
  ...
  (t) => [
    index("chat_memory_events_session_turn_idx").on(
      t.sessionId,
      t.turnIndex,
      t.createdAt,
    ),
    index("chat_memory_events_distinct_created_idx").on(
      t.distinctId,
      t.createdAt,
    ),
    index("chat_memory_events_key_created_idx").on(
      t.memoryNamespace,
      t.memoryKey,
      t.createdAt,
    ),
    ...
    check(
      ...
      sql`${t.memoryScope} in ('session','cross_session')`,
    ),
    check(
    ...
    check(
      ...
      sql`${t.confidenceCap} in ('none','medium','low')`,
    ),
    check(
...
export const chatSessionMemory = createTable(
  ...
  (t) => [
    ...
    index("chat_session_memory_session_updated_idx").on(
      t.sessionId,
      t.lastUpdatedAt,
    ),
...
export const chatUserPreferences = createTable(
  ...
  (t) => [
    ...
    index("chat_user_preferences_distinct_updated_idx").on(
      t.distinctId,
      t.lastUpdatedAt,
    ),
...
export const chatCompetitorChecks = createTable(
  ...
  (t) => [
    index("chat_competitor_checks_session_turn_created_idx").on(
      t.sessionId,
      t.turnIndex,
      t.createdAt,
    ),
...
export const chatReviewSignalRuns = createTable(
  ...
  (t) => [
    index("chat_review_signal_runs_session_turn_created_idx").on(
      t.sessionId,
      t.turnIndex,
      t.createdAt,
    ),
    index("chat_review_signal_runs_place_created_idx").on(
      t.placeId,
      t.createdAt,
    ),
    check(
...
export const chatRecommendationEvidence = createTable(
  ...
  (t) => [
    ...
    index("chat_recommendation_evidence_place_publish_idx").on(
      t.placeId,
      t.reviewPublishAt,
    ),
    check(
